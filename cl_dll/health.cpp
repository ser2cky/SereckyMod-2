...
int CHudHealth::Draw(float flTime)
{
	...
	if (gHUD.m_iWeaponBits & (1<<(WEAPON_SUIT)))
	{
		...
		y = gHUD.m_iScreenHeight - gHUD.m_iFontHeight - gHUD.m_iFontHeight / 2;
		x = CrossWidth /2;
...
int CHudHealth::DrawPain(float flTime)
{
	...
	if (m_fAttackFront > 0.4)
	{
		...
		x = gHUD.m_iScreenWidth/2 - SPR_Width(m_hSprite, 0)/2;
		y = gHUD.m_iScreenHeight/2 - SPR_Height(m_hSprite,0) * 3;
	...
	if (m_fAttackRight > 0.4)
	{
		...
		x = gHUD.m_iScreenWidth/2 + SPR_Width(m_hSprite, 1) * 2;
		y = gHUD.m_iScreenHeight/2 - SPR_Height(m_hSprite,1)/2;
	...
	if (m_fAttackRear > 0.4)
	{
		...
		x = gHUD.m_iScreenWidth/2 - SPR_Width(m_hSprite, 2)/2;
		y = gHUD.m_iScreenHeight/2 + SPR_Height(m_hSprite,2) * 2;
	...
	if (m_fAttackLeft > 0.4)
	{
		...
		x = gHUD.m_iScreenWidth/2 - SPR_Width(m_hSprite, 3) * 3;
		y = gHUD.m_iScreenHeight/2 - SPR_Height(m_hSprite,3)/2;
...
{	
	...
	for (int i = 0; i < NUM_DMG_TYPES; i++)
	{
		...
		if (bitsOn & giDmgFlags[i])
		{
			...
			pdmg->x = giDmgWidth/8;
			pdmg->y = gHUD.m_iScreenHeight - giDmgHeight * 2;
